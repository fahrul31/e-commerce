...
// Endpoint untuk mengirim komplain dan mengubah status pesanan menjadi returned
export async function PATCH(request, { params }) {
    ...
    const { complaint_reason } = await request.json();


    if (!complaint_reason) {
        return NextResponse.json(
            { success: false, message: "Alasan komplain harus disertakan" },
            { status: 400 }
        );
    }
    ...
    try {
        // Update status order menjadi 'returned' dan simpan alasan komplain
        const updateQuery = `
            UPDATE orders
            SET status = 'returned', complaint_reason = ?
            WHERE id = ?
        `;
        const [result] = await db.query(updateQuery, [complaint_reason, orderId]);
        ...
        // Email ke admin setelah komplain diterima
        const adminMailOptions = {
            from: process.env.EMAIL,
            to: process.env.ADMIN_EMAIL,
            subject: `Komplain Pesanan - Order ID: ${orderId}`,
            html: `
                <p>Pesanan dengan Order ID: <strong>${orderId}</strong> telah dikomplain oleh pembeli <strong>${order.customer_name}</strong> (${order.email})</p>
                <p><strong>Alasan Komplain:</strong> ${complaint_reason}</p>
        ...
        return NextResponse.json({
            success: true,
            message: "Pesanan telah dikomplain dan status diubah menjadi 'returned'",
        });
    } catch (error) {
